...
public class InitalMethodRaceDetector implements RaceDetector<MethodRace> {
    ...
    private Map<FieldId, Set<MethodStmtPair>> readIfGaurdMap = new HashMap<>();
    private Map<FieldId, List<MethodStmtPair>> readFieldIdToMethods = new HashMap<>();
    private Set<FieldIdMethodPair> writeNullAtEndSet = new HashSet<>();
    private Map<FieldId, Set<FieldId>> nonAllocToAllAllocFieldIds = new HashMap<>();
    private Set<MethodRace> methodRaceSet = new HashSet<>();
    public InitalMethodRaceDetector(CSFManager myCaSFManager) {
        LOG.startTimer("InitalMethodRaceDetector", LOG.VERBOSE);
        for (MethodBox methodBox : myCaSFManager.getAllMethodBoxes()) {
            if (methodBox.getIntraAllocationAnalysis() == null)
                continue;
            ...
            for (MethodStmtPair writePair : writeFieldIdToMethods.getOrDefault(fieldId, new ArrayList<>())) {
                for (MethodStmtPair readPair : readFieldIdToMethods.get(fieldId)) {
                    ...
                    MethodRace methodRace = new MethodRace(writePair.getMethod(), writePair.getStmt(), readPair.getMethod(), readPair.getStmt(), fieldId)
                            .setUAF(true)
                            .setIfGuard(readIfGaurdMap.containsKey(fieldId) && readIfGaurdMap.get(fieldId).contains(readPair))
                            .setNullAtEnd(writeNullAtEndSet.contains(new FieldIdMethodPair(fieldId, writePair.getMethod())))
        ...
        LOG.logln(String.format("ER: %d, UAF: %d, IfGuard: %d, NullAtEnd: %d, nAdroid: %d, Read Fields: %d Write Fields %d", methodRaceSet.size(), totalUAF, totalIfGuard, totalNullAtEnd, nAdroid, readFieldIdToMethods.size(), writeFieldIdToMethods.size()), LOG.ESSENTIAL);
        LOG.endTimer("InitalMethodRaceDetector", LOG.VERBOSE);
    ...
    private void addReadAccess(Local base, IntraAllocationAnalysis intraAllocationAnalysis, Unit unit, SootMethod sootMethod, DominatorTree<Unit> dTree) {
        ...
        final boolean[] ifGuard = {false};
        if(dTree != null) {
            // TODO: check if one of the current unit is the memory location unit of base
            while (currentUnit != null) {
                if (currentUnit instanceof IfStmt) {
                    IfStmt ifStmt = (IfStmt) currentUnit;
                    ifStmt.getCondition().apply(new AbstractJimpleValueSwitch() {
                        @Override
                        public void caseCmpExpr(CmpExpr v) {
                            if(v.getOp1().equals(NullConstant.v())){
                                Scene.v();
                            }
                            if(v.getOp2().equals(NullConstant.v())){
                                if(v.getOp1().equals(base))
                                    Scene.v();
                            }
                            Scene.v();
                        }
...
//                if(ifGuard[0])
//                    return;
                DominatorNode<Unit> parent = dTree.getParentOf(dTree.getDode(currentUnit));
                if (parent == null)
                    break;
                currentUnit = parent.getGode();
            }
        }
...
//                    continue;
                ...
                for(FieldId fieldId : fieldIds){
                    ...
                    if(ifGuard[0]) {
                        if(!readIfGaurdMap.containsKey(fieldId))
                            readIfGaurdMap.put(fieldId, new HashSet<>());
                        readIfGaurdMap.get(fieldId).add(readMSPair);
                    }
...
//                intraAllocationAnalysis.
        ...
        for (MemoryPair memoryPair : intraAllocationAnalysis.getEndMemPairs()) {
            if (memoryPair.getAddr() instanceof FieldMemAddr) {
                if (!memoryPair.getLoc().equals(NullAllocMemLoc.v()))
                    continue;
                MemoryLocation fieldMemLoc = ((FieldMemAddr) memoryPair.getAddr()).getFieldMemLoc();
                ...
                Set<FieldId> fieldIdSet = getFieldIds(fieldSequence);
                for(FieldId fieldId : fieldIdSet){
                    writeNullAtEndSet.add(new FieldIdMethodPair(fieldId, sootMethod));
                }
            }
        }
    }
