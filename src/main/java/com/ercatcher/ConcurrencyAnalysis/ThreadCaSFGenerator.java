...
public class ThreadCaSFGenerator implements LibraryCaSFGenerator {
    ...
    public ThreadCaSFGenerator(SootClass sootClass) { }

    public boolean canDetectTarget(MethodBox sourceBox, UInvokeNode uInvokeNode, MethodBox targetBox){
        if(!targetBox.getSource().getSignature().equals("<java.lang.Thread: void run()>"))
            return false;
        ...
        if(Util.v().isSubclass(base.getType(), "android.os.HandlerThread"))
            return false;
        return true;
    }
